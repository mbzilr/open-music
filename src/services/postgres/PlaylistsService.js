...
class PlaylistsService {
  ...
  async addPlaylist({ name, owner }) {
    ...
    if (!owner) {
      throw new AuthenticationError('Anda harus login untuk membuat playlist');
    }
  ...
  async addSongToPlaylist(playlistId, songId, userId) {
    const checkedSong = await this._pool.query('SELECT id FROM songs WHERE id = $1', [songId]);
    if (!checkedSong.rowCount) {
